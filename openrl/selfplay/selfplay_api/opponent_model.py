...
class BattleHistory:
    ...
    def update_nonzero(self):
        self.num_nonzero_rewards += 1
    def get_battle_info(self) -> Dict[str, Any]:
        ...
        result["non_zero_rate"] = float(self.num_nonzero_rewards) / max(self.num_games, 1)
        result['total_reward'] = sum(self.rewards)
        result['avg_reward'] = sum(self.rewards) / max(self.num_games, 1)
...
class OpponentModel:
    ...
    def update_trueskill(self,result: BattleResult,rating):
        if result == BattleResult.WIN:
            winner, loser = trueskill.rate([self.rating, rating], ranks=[0, 1])
            self.rating = winner
        elif result == BattleResult.LOSE:
            winner, loser = trueskill.rate([rating,self.rating], ranks=[0, 1])
            self.rating = loser
        elif result == BattleResult.DRAW:
            player1, player2 = trueskill.rate([self.rating, rating], ranks=[0, 0])
            self.rating = player1
